...
export function Navbar() {
  ...
  return (
    <nav className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[96%] max-w-7xl animate-fade-in-nav">
      <div className="bg-card/95 backdrop-blur-xl border border-border/30 rounded-2xl px-4 md:px-6 py-3 shadow-lg shadow-primary/5 w-full mx-auto">
        <div className="flex items-center justify-between max-w-7xl mx-auto">
          ...
          <div className="hidden md:flex items-center space-x-2 flex-shrink-0">
            <Button
              asChild
              className={cn(
                "bg-primary hover:bg-primary/90 text-primary-foreground rounded-full px-6 transition-all duration-200 hover:scale-105",
          ...
          <div className="flex md:hidden items-center space-x-2">
            <Button
              ...
              size="sm"
              className={cn(
                "bg-primary hover:bg-primary/90 text-primary-foreground rounded-full px-4",
                pathname === "/auth/sign-up?role=student" &&
      ...
      {isOpen && (
        <div className="md:hidden absolute top-20 inset-x-0 z-40 px-4">
          <div className="bg-card backdrop-blur-xl border border-border rounded-2xl p-4 shadow-2xl max-h-[calc(100vh-6rem)] overflow-y-auto">
            {[...mainNavItems, ...legalNavItems].map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className={cn(
                  "text-foreground hover:text-primary hover:bg-muted/50 block px-3 py-2 rounded-md text-base font-medium flex items-center",
                  pathname === item.href && "text-primary bg-primary/10",
                )}
                onClick={closeMenu}
              >
            ...
            <div className="px-3 py-2">
              <a
                href={telLink}
                className="text-foreground hover:text-primary block py-2 rounded-md text-base font-medium flex items-center"
                onClick={closeMenu}
              >
